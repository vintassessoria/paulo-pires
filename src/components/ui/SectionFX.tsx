...
export default function SectionFX({ variant = 'washes' }: { variant?: Variant }) {
  return (
    <div aria-hidden className="pointer-events-none absolute inset-0 -z-10 overflow-hidden">
      {variant === 'washes' && (
        <>
          <div
            ...
            style={{ background: 'radial-gradient(circle, rgba(178,58,18,0.5), transparent 62%)' }}
          />
          <div
            className="animate-drift2 absolute -right-[10%] bottom-[-15%] h-[60vh] w-[60vh] rounded-full opacity-60 blur-[100px]"
            style={{ background: 'radial-gradient(circle, rgba(210,80,28,0.5), transparent 62%)' }}
          />
          <div
            className="animate-drift3 absolute left-[35%] top-[20%] h-[50vh] w-[50vh] rounded-full opacity-50 blur-[110px]"
            style={{ background: 'radial-gradient(circle, rgba(150,60,26,0.5), transparent 62%)' }}
          />
        </>
      )}
      ...
      {variant === 'orbs' &&
        [
          { c: 'animate-drift1', pos: 'left-[8%] top-[14%]', size: 'h-44 w-44', col: 'rgba(255,90,31,0.42)' },
          { c: 'animate-drift3', pos: 'right-[12%] top-[22%]', size: 'h-28 w-28', col: 'rgba(255,122,72,0.36)' },
          { c: 'animate-drift2', pos: 'left-[24%] bottom-[14%]', size: 'h-56 w-56', col: 'rgba(160,58,22,0.42)' },
          { c: 'animate-drift1', pos: 'right-[6%] bottom-[8%]', size: 'h-36 w-36', col: 'rgba(210,80,28,0.4)' },
          { c: 'animate-drift3', pos: 'left-[52%] top-[6%]', size: 'h-24 w-24', col: 'rgba(255,122,72,0.3)' },
        ].map((o, i) => (
          <div
            key={i}
      ...
      {variant === 'aurora' && (
        <>
          <div
            ...
            style={{
              background:
                'linear-gradient(90deg, transparent, rgba(255,90,31,0.5), rgba(255,122,72,0.4), transparent)',
            }}
          />
          <div
            className="animate-wave absolute left-[-20%] top-[46%] h-32 w-[140%] rotate-3 opacity-50 blur-[80px]"
            style={{
              background: 'linear-gradient(90deg, transparent, rgba(210,80,28,0.5), transparent)',
              animationDelay: '-4s',
          ...
          <div
            ...
            style={{
              background: 'linear-gradient(90deg, transparent, rgba(160,58,22,0.5), transparent)',
              animationDelay: '-9s',
      ...
      {variant === 'beam' && (
        <>
          <div
            ...
            style={{
              background:
                'conic-gradient(from 0deg, transparent 0deg, rgba(255,90,31,0.35) 40deg, transparent 95deg, transparent 205deg, rgba(210,80,28,0.3) 250deg, transparent 305deg)',
          ...
          <div
            ...
            style={{ background: 'radial-gradient(circle, rgba(178,58,18,0.4), transparent 65%)' }}
          />
        </>
      )}
    </div>
